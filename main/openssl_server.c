...
static uint8_t led_pin[] = {2,0};
static int led_cnt = sizeof(led_pin)/sizeof(led_pin[0]);

static uint8_t pir_pin; //pin is GPIO5
static uint8_t h, l;
static long lux;
...
reconnect:
    ...
    do {
        ...
        for (int i=0;i<led_cnt;i+=2) {
        	...
        	ESP_LOGI(TAG, "form_text %s", form_text);
        	ESP_LOGI(TAG, "against recv_buf %s", recv_buf);
        ...
        if (strncmp(recv_buf, "GET / HTTP/1.1",14)==0) {
        	ESP_LOGI(TAG, "GETROOT  %s", recv_buf);
        	openssl_demo_main=1;
        }
        if (openssl_demo_main==1) {
        	ESP_LOGI(TAG, "openssl_demo_main");
        	openssl_demo_write(ssl, OPENSSL_DEMO_HTTP_200_TXT,NULL,NULL);
        	openssl_demo_write(ssl, OPENSSL_DEMO_HTTP_HEAD,"{v}","Basheer's ESP32 SSL Demo");
            ...
            for (int i=0;i<led_cnt;i+=2) {
            	...
            	sprintf(button_text,"GPIO %d %s",led_pin[i],led_pin[i+1]==0?"is off":"is on");
        		...
        		ESP_LOGI(TAG, "openssl_demo_write3");
        	}
            // write PIR status into button for GPIO5
            pir_pin = gpio_get_level(5);
            // read lux
			lux = read_current_lux(&h, &l);

        	sprintf(form_text,"PIR");
        	sprintf(button_text,"PIR %s lux %ld",pir_pin==0?"is off":"is on", lux);
    		openssl_demo_write3(ssl, pir_pin==0?OPENSSL_DEMO_HTTP_FORM_BUTTON_ON:OPENSSL_DEMO_HTTP_FORM_BUTTON_OFF,"{f}",form_text,"{l}",button_text,NULL,NULL);
    		ESP_LOGI(TAG, "openssl_demo_write3");

        	openssl_demo_write(ssl, OPENSSL_DEMO_HTTP_END,NULL,NULL);
        	ESP_LOGI(TAG, "OPENSSL_DEMO_HTTP_END");
            break;
        }
        if (strncmp(recv_buf, "GET /logo.png HTTP/1.1",22)==0) {
            ...
            ESP_LOGI(TAG, "logo.png");
            break;
        }
        else // other request, like GET favicon.ico HTTP/1.1
        {
        	openssl_demo_write(ssl, OPENSSL_DEMO_HTTP_404,NULL,NULL);
        	break;
        }
    } while (1);
